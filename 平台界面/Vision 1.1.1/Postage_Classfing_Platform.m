...
% Last Modified by GUIDE v2.5 27-Nov-2016 14:51:27
...
global Img_data Img_pro_data;
val = get(hObject,'value');
handles.angle = val;
if val == 0
    val=1;
end
set(handles.edit1,'string',num2str(floor(val)));
i=floor(val);

imshow(rot90(Img_data.temp_data(:,:,i)),'Parent',handles.show1);
imshow(rot90(Img_pro_data.temp_data(:,:,i)),'Parent',handles.show3);
set(handles.edit8,'string',num2str(i));
% imshow(rot90(Img_data.result()))
guidata(hObject, handles);
...
set(handles.slider1,'Max',z);%还没与解决最大值的问题，怀疑是matlab平台的问题。
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[name, path] = uigetfile({'*.jpg';'*.png';'*.bmp'},'打开图像','MultiSelect', 'on');
[a,num]=size(name);
% set(handles.edit9,'string',[path,name(1)]);
str=[10];
for i=1:num
    str=[str,[path,char(name(i))],10];
    train_img(i).data=load([path,char(name(i))]);
end
set(handles.edit9,'string',str,'FontSize',12);
...
% handles    structure with handles and user data (see GUIDATA)
global Img_data;
[name, path] = uigetfile({'*.mat';'*.jpg';'*.png';'*.bmp'},'打开图像');
% handles.name=name;
% handles.path=path;
Img_data = load([path,name]);
MAX=max(max(max(Img_data.temp_data)));
Img_data.temp_data=Img_data.temp_data./MAX;%归一化处理
imshow(rot90(Img_data.temp_data(:,:,1)),'Parent',handles.show1);%图像旋转显示。
handles.image=Img_data.temp_data;
guidata(hObject, handles);

% %显示专家图像。
A=isstrprop(name,'digit'); %提取数字
B=name(A);
C=str2num(B);
global Img_pro_data
Img_pro_data = load([path,'img',num2str(C),'_pro.mat']);
imshow(rot90(Img_pro_data.temp_data(:,:,1)),'Parent',handles.show3);

[x,y,z]=size(Img_data.temp_data);
set(handles.edit6,'string',num2str(x));%将图像的三维放入xyz中。
set(handles.edit7,'string',num2str(y));
set(handles.edit8,'string',num2str(z));
set(handles.slider1,'max',z);%设置滑条最大值
...
% --- Executes during object creation, after setting all properties.
function show3_CreateFcn(hObject, eventdata, handles)
% hObject    handle to show3 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% Hint: place code in OpeningFcn to populate show1
