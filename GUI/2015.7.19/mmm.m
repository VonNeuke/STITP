...
% load test_phantom_zhou2;%%导入周建的phantom
...
pro_moments=pro_mom(sino,order);%计算所知角度的投影矩
%下面是计算系数矩阵T
M=0:(order+1);
M=sum(M);
T_matrix=zeros(na*(order+1),M);
T_matrix=get_T(theta,order);%计算系数矩阵
fi_matrix=reshape(pro_moments(1:(order+1),:)',na*(order+1),1);%将投影矩拉直
%以符合矩阵计算的需要
