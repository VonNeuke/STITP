...
%存储文件，节省内存
save_data(path_position,temp_pos,n_polution_data+1);
save_data(path_label,temp_label,n_polution_data+1);
clear temp_pos temp_label
end 
%compute haar features
disp('计算初始特征，开始...')
if ~exist([path_context_features,num2str(n_polution_data+1),'.mat'],'file')
temp_context_feature = cal_context_features( image , all_pos );
save_data(path_context_features,temp_context_feature,n_polution_data+1);
clear temp_context_feature
end

if ~exist([path_haar_features,num2str(n_polution_data+1),'.mat'],'file')
temp_haar_feature = cal_haar_features_3D( image , all_pos );
save_data(path_haar_features,temp_haar_feature,n_polution_data+1);
...
for i=2:n_inti
    ...
    %计算随机采样点和标签
    if ~exist([path_label,num2str(i+n_polution_data),'.mat'],'file')%&&~exit(path_position,'file')
    [ temp_pos , temp_label] = sample_pos(label_image,image,n_sample,'index');
    ...
    %计算haar feature 
    if ~exist([path_haar_features,num2str(i+n_polution_data),'.mat'],'file')
    temp_haar_feature = cal_haar_features_3D( image , all_pos );
    save_data(path_haar_features,temp_haar_feature,i+n_polution_data);
    end
    clear temp_haar_feature
    %计算context_feature
    if ~exist([path_context_features,num2str(i+n_polution_data),'.mat'],'file')
    temp_context_feature = cal_context_features( image , all_pos );
...
if~exist([path_model,'model_Day',num2str(1),'.mat'],'file')
    ...
    save([path_model,'model_Day',num2str(1),'.mat'],'model');
else
    load([path_model,'model_Day',num2str(1),'.mat'],'model');
end
